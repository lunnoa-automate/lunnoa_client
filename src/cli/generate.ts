...
/** Stable fingerprint of entity-type schemas for codegen freshness checks. */
export function computeDeploymentSchemaFingerprint(
  entityTypes: EntityTypeDefinition[],
): string {
  const parts = [...entityTypes]
    .map((type) => {
      const revision = type.schemaRevision ?? 1;
      const allowUnknown = type.allowUnknownAttributes ?? false;
      return `${type.slug}:${revision}:${allowUnknown ? 1 : 0}`;
    })
    .sort();
...
export function generateDeploymentModule(
  ...
  const schemaFingerprint = computeDeploymentSchemaFingerprint(
  ...
  chunks.push(
    `/* eslint-disable */\n` +
      ...
      `\n` +
      `/** Fingerprint of entity-type schemas at codegen time (slug:revision:allowUnknown). */\n` +
      `export const DEPLOYMENT_SCHEMA_FINGERPRINT = ${JSON.stringify(schemaFingerprint)};\n` +
      `\n` +
      `/** Per-type schemaRevision values captured at codegen time. */\n` +
      `export const ENTITY_TYPE_SCHEMA_REVISIONS = ${JSON.stringify(
        Object.fromEntries(
          definitions.entityTypes.map((type) => [
            type.slug,
            type.schemaRevision ?? 1,
          ]),
        ),
      )} as const;\n`,
  );
